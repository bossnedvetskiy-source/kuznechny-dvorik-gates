...
test('real color photo follows customer into calculator and lead', async ({page}) => {
  ...
  await mint.click();
  await expect(card.locator('.profile-color-status')).toContainText('Выбран: Зелёная мята');
  await expect(card.locator('.profile-color-status')).toContainText('отдельного фото');
