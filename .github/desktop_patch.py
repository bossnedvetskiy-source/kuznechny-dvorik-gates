...
s = replace_once(
    s,
    '''      <div class="hero-prices" aria-label="Минимальная стоимость стандартного комплекта">\n        <div><small>На готовые столбы</small><strong id="heroInstalledPrice">от 64 600 ₽</strong><span>ворота, калитка и монтаж</span></div>\n        <div><small>Полностью под ключ</small><strong id="heroTurnkeyPrice">от 89 600 ₽</strong><span>с новыми усиленными столбами</span></div>\n      </div>''',
    '''      <div class="hero-prices" aria-label="Минимальная стоимость стандартного комплекта">\n        <div><small>На готовые столбы</small><strong id="heroInstalledPrice">от 64 600 ₽</strong><span>ворота, калитка и монтаж</span></div>\n        <div><small>Полностью под ключ</small><strong id="heroTurnkeyPrice">от 89 600 ₽</strong><span>с новыми усиленными столбами</span></div>\n      </div>\n      <div class="hero-size-note">Цены указаны для стандартного размера: ворота 3,4×1,8 м и калитка 1×1,8 м. Другие размеры можно рассчитать на сайте.</div>''',
    'hero size note'
)
s = s.replace('<span id="catalogCount">40 моделей</span>', '<span id="catalogCount">38 моделей</span>')
s = s.replace('<span id="catalogProgress">Показано 6 из 40</span>', '<span id="catalogProgress">Показано 6 из 38</span>')
s = s.replace('Стандарт: ворота 3,4×1,8 м и калитка 1×1,8 м. Монтаж, новые столбы и доставка выбираются в калькуляторе.', 'Стандарт: ворота 3,4×1,8 м и калитка 1×1,8 м. В базовую цену входит установка на готовые столбы. Новые усиленные столбы и доставка рассчитываются отдельно.')
s = s.replace('<div><span class="section-number">03</span><h2>Предварительная смета</h2></div>', '<div><h2>Расчёт выбранной модели</h2></div>')
s = s.replace('<div class="mobile-lead-heading"><span>Последний шаг</span><h3>Получить точную стоимость</h3><p>Укажите телефон — согласуем бесплатный замер и окончательную цену.</p></div>', '<div class="mobile-lead-heading"><span>Последний шаг</span><h3>Заказать бесплатный замер</h3><p>Оставьте телефон — мы свяжемся, согласуем замер и окончательную стоимость.</p></div>')
s = s.replace('<button class="button button-primary wide" id="sendButton" type="button">Отправить заявку в WhatsApp</button>', '<button class="button button-primary wide" id="sendButton" type="button">Отправить заявку</button>')
s = s.replace('После нажатия заявка сохраняется в защищённой админ-панели и открывается готовое сообщение в WhatsApp.', 'После отправки заявка сохранится в защищённой админ-панели. Написать в WhatsApp можно будет по желанию.')
s = s.replace('<span class="section-number">04</span>\n        <h2>Реальные работы.', '<span class="section-number">03</span>\n        <h2>Реальные работы.')
s = s.replace('<div><span class="section-number">05</span><h2>Ответы на вопросы</h2></div>', '<div><span class="section-number">04</span><h2>Ответы на вопросы</h2></div>')
s = s.replace('Да. Стандарт — ворота 3,4×1,8 м и калитка 1×1,8 м. Другой размер рассчитаем после бесплатного замера.', 'Да. Стандарт — ворота 3,4×1,8 м и калитка 1×1,8 м. Измените размеры в калькуляторе — стоимость пересчитается сразу. Окончательно подтвердим размеры и цену после бесплатного замера.')
p.write_text(s, encoding='utf-8')


# --- storefront.css desktop readability ---
p = Path('storefront.css')
css = p.read_text(encoding='utf-8')
marker = '/* Desktop UX sync 2026-09 */'
if marker not in css:
    css += r'''

/* Desktop UX sync 2026-09 */
@media(min-width:621px){
  .hero-size-note{margin:10px 0 0!important;color:rgba(255,255,255,.48)!important;font-size:11px!important;line-height:1.45!important;max-width:560px!important}
  .product-meta span{font-size:10px!important}
  .price-row small{font-size:11px!important;min-height:30px!important}
  .estimate-line{font-size:13px!important}
  .estimate-note{font-size:11px!important}
  .consent-row{font-size:11px!important}
  .product-picker-label{display:none!important}
  .desktop-change-model{margin-top:7px;border:0;background:transparent;color:#d9af5e;font-size:12px;font-weight:800;padding:0;cursor:pointer;text-align:left}
  .desktop-change-model:hover{text-decoration:underline}
}
@media(max-width:620px){.hero-size-note{display:none!important}}
'''
p.write_text(css, encoding='utf-8')


# --- public-site-settings.js desktop interaction layer ---
p = Path('public-site-settings.js')
js = p.read_text(encoding='utf-8')
marker = '// Desktop UX sync with mobile flow 2026-09'
if marker not in js:
    block = r'''
...
'''
    pos = js.rfind('})();')
    if pos < 0:
        raise SystemExit('public-site-settings closing marker not found')
    js = js[:pos] + block + '\n' + js[pos:]
