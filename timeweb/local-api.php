...
{
    $limit=max(1,min(200,(int)($_GET['limit']??50)));$before=(int)($_GET['before_id']??0);$status=trim((string)($_GET['status']??''));$q=trim((string)($_GET['q']??''));$source=trim((string)($_GET['source']??''));$category=trim((string)($_GET['category']??''));$from=(string)($_GET['from']??'');$to=(string)($_GET['to']??'');
    $where=[];$args=[];if($before>0){$where[]='id < ?';$args[]=$before;}if(in_array($status,['new','contacted','done','archived'],true)){$where[]='status=?';$args[]=$status;}if($source!==''){$where[]='source=?';$args[]=$source;}if($category!==''){$where[]='category=?';$args[]=$category;}if(preg_match('/^\d{4}-\d{2}-\d{2}$/',$from)){$where[]='created_at>=?';$args[]=$from.' 00:00:00';}if(preg_match('/^\d{4}-\d{2}-\d{2}$/',$to)){$where[]='created_at<DATE_ADD(?,INTERVAL 1 DAY)';$args[]=$to.' 00:00:00';}
    if($q!==''){$where[]='(name LIKE ? OR city LIKE ? OR article LIKE ? OR source LIKE ? OR product_title LIKE ? OR phone LIKE ? OR category LIKE ?)';$like='%'.$q.'%';array_push($args,$like,$like,$like,$like,$like,$like,$like);}
    ...
    $leads=array_map('kd_lead_row',$rows);$counts=['new'=>0,'contacted'=>0,'done'=>0,'archived'=>0];foreach(kd_db()->query('SELECT status,COUNT(*) c FROM site_leads GROUP BY status')->fetchAll() as $r)if(isset($counts[$r['status']]))$counts[$r['status']]=(int)$r['c'];$sources=[];foreach(kd_db()->query("SELECT source,COUNT(*) c FROM site_leads WHERE TRIM(source)<>'' GROUP BY source ORDER BY c DESC,source ASC LIMIT 100")->fetchAll() as $r)$sources[]=['value'=>$r['source'],'count'=>(int)$r['c']];$categories=[];foreach(kd_db()->query("SELECT category,COUNT(*) c FROM site_leads WHERE TRIM(category)<>'' GROUP BY category ORDER BY c DESC,category ASC LIMIT 100")->fetchAll() as $r)$categories[]=['value'=>$r['category'],'count'=>(int)$r['c']];
    kd_json(['leads'=>$leads,'counts'=>$counts,'sources'=>$sources,'categories'=>$categories,'limited'=>false,'page'=>['limit'=>$limit,'beforeId'=>$before?:null,'nextBeforeId'=>$hasMore&&$leads?(int)end($leads)['id']:null,'hasMore'=>$hasMore,'status'=>$status?:'all','q'=>$q,'source'=>$source,'category'=>$category,'from'=>$from,'to'=>$to],'totalCount'=>array_sum($counts),'filteredTotal'=>count($leads)]);
}
