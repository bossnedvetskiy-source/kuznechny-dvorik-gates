...
function kd_admin_leads(): never
{
    ...
    kd_json(['leads'=>$leads,'counts'=>$counts,'trustCounts'=>$trustCounts,'sources'=>$sources,'limited'=>false,'page'=>['limit'=>$limit,'beforeId'=>$before?:null,'nextBeforeId'=>$hasMore&&$leads?(int)end($leads)['id']:null,'hasMore'=>$hasMore,'status'=>$status?:'all','q'=>$q,'source'=>$source,'from'=>$from,'to'=>$to],'totalCount'=>array_sum($counts),'filteredTotal'=>count($leads)]);
}
