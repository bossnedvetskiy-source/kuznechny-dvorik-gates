...
(() => {
  ...
  const cleanAdministrativeAddress = value => String(value || '')
    ...
    .map(part => part.trim())
    .filter(part => part
      && !/^(?:город|село|деревня|хутор|город\s*\/\s*пос[её]лок|пос[её]лок|село\s*\/\s*деревня|деревня\s*\/\s*хутор|насел[её]нный пункт)$/iu.test(part)
...
(() => {
  ...
  window.KUZDVOR_ENSURE_CALCULATOR = () => {
    ...
    calculatorPromise = loadScript('/calculator.bundle.js?v=6dd4db0e8dc7')
      .then(() => {
  ...
  const startCatalogDeferredWork = () => {
    ...
    loadScript('/catalog-enhancements.bundle.js?v=6dd4db0e8dc7').catch(error => console.error('Catalog enhancements load failed', error));
