...
handleAdminApi = async function handleAdminApiWithAtomicColorUploads(request, env, url) {
  ...
  if (!path.startsWith(uploadPrefix) && !path.startsWith(slotPrefix)) return __handleAdminApiWithoutAtomicColorUploads(request,env,url);
  ...
  const prefix=path.startsWith(uploadPrefix)?uploadPrefix:slotPrefix;
  let remainder;
  try { remainder=decodeURIComponent(path.slice(prefix.length)); } catch { return json({error:'Некорректный адрес'},400); }
